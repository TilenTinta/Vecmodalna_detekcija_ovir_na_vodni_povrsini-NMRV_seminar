...
from network import SegFormerModel
...
def visualize_prediction(model, dataloader, device):
    ...
    # Ensure images is a batch
    if len(images.shape) == 3:  # If single image [C, H, W], add batch dimension
...
def main():
    ...
    model_weights = os.path.join(data_root, 'SegFormer_Thermal_best.pth')
    image_size = (384, 288)
    ...
    # Naloži model
    model = SegFormerModel(num_classes=4).to(device) # Init SegFormerja
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            outputs = model(images)  # Direct logits
            predictions = torch.argmax(outputs, dim=1)


            # Ignore class labels 6 and 7 in evaluation
            masks[masks == 255] = -1  # Ignore in IoU computation
            valid_mask = (masks >= 0)  # Only keep valid classes
