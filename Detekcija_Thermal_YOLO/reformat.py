...
def copyImg():
    image_dir = "..\\NMRV_seminar\\thermal_images"
    ...
    def resize_image(src_path, dst_path, size=(640, 360)):
        if os.path.exists(src_path):
            ...
            # Če je 16-bit grayscale slika
            if img is not None and len(img.shape) == 2 and img.dtype == np.uint16:
                img = cv2.normalize(img, None, alpha=0, beta=65535, norm_type=cv2.NORM_MINMAX)
                img = cv2.convertScaleAbs(img, alpha=255.0 / 65535.0)  # 16-bit to 8-bit

            # Če je grayscale, pretvori v RGB
            if img is not None and len(img.shape) == 2:
                img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)

            # Če je vse ok, spremeni velikost in shrani
            if img is not None:
...
def convert_annotations(json_file, image_folder, output_folder, resized_size=(640, 360)):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json", type=str, default="..\\NMRV_seminar\\thermal_annotations.json", help="Path to JSON annotation file")
    parser.add_argument("--images", type=str, default="..\\NMRV_seminar\\thermal_images", help="Folder containing images")
